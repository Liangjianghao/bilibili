...
def addVideo():
    ...
    print(len(data))
    for video in data:
        id=video['videoUrl'].split('av')[1]
        # print(id)
        numberArr.append(id)
    print(len(numberArr))
    print(type(numberArr[0]))
    for x in range(1,33000000):
        if str(x) in numberArr:
            print(x)
        else:
            url='https://www.bilibili.com/video/av%s'%str(x)
            f.write(url+'\n')
    f.close()
print('finish')
